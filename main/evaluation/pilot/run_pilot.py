...
Source images live OUTSIDE this repo -- a separate, unrelated crawl from
another personal project (C:/Users/smn07/Desktop/glacier-project/itda),
organized as <root>/<source folder>/<korean category>/<product name>.jpg.
Not part of the reusable evaluation/ package: this is a one-time data
preparation + experiment-running step for a specific real-data pilot.
...
FOLDER_TO_COLLECTION = {
    ("musinsa_pants_1000", "청바지"): "pants",
    ("musinsa_pants_1000", "바지"): "pants",
    ("musinsa_upper_2000", "긴팔"): "top",
    ("musinsa_upper_2000", "니트"): "top",
    ("musinsa_upper_2000", "맨투맨"): "top",
    ("musinsa_upper_2000", "반팔"): "top",
    ("musinsa_upper_2000", "셔츠"): "top",
    ("musinsa_upper_2000", "나시"): "top",
    ("musinsa_upper_2000", "가디건"): "outer",
    ("musinsa_upper_2000", "후드티"): "outer",
}
...
# (query_id, subroot, subdir, filename, category, difficulty, scene_type, num_visible_items, background_complexity)
QUERY_SPECS = [
    ("Q001", "musinsa_pants_1000", "청바지", "062 테이퍼드 핏 데님팬츠 블랙진.jpg", "pants", "hard", "person_wearing", 2, "low"),
    ("Q002", "musinsa_pants_1000", "청바지", "507 와이드 데님 - 화이트.jpg", "pants", "easy", "clean_product", 1, "low"),
    ("Q003", "musinsa_upper_2000", "니트", "ANGORA PULLOVER KNIT_CREAM.jpg", "top", "hard", "person_wearing", 2, "medium"),
    ("Q004", "musinsa_upper_2000", "반팔", "소사이어티 티셔츠 베이지.jpg", "top", "easy", "clean_product", 1, "low"),
    ("Q005", "musinsa_upper_2000", "맨투맨", "ATHLETE STRIPE LS - [IVORY].jpg", "top", "medium", "clean_product", 1, "low"),
    ("Q006", "musinsa_upper_2000", "가디건", "데일리 소프트 가디건&풀오버 SET 4종(택2).jpg", "outer", "hard", "multi_item", 2, "high"),
    ("Q007", "musinsa_upper_2000", "후드티", "2010 Sports Hoodie Grey.jpg", "outer", "medium", "clean_product", 1, "low"),
]
...
def gather_catalog_paths(exclude_paths):
    ...
    for (subroot, subdir), collection in FOLDER_TO_COLLECTION.items():
        folder = SOURCE_ROOT / subroot / subdir
        for path in sorted(folder.iterdir()):
            if path.is_file() and str(path.resolve()) not in exclude:
...
def main():
    ...
    query_paths = [SOURCE_ROOT / s[1] / s[2] / s[3] for s in QUERY_SPECS]
    ...
    for spec, query_path in zip(QUERY_SPECS, query_paths):
        query_id, subroot, subdir, filename, category, difficulty, scene_type, num_items, bg_complexity = spec
